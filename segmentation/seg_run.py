...
direct = './path_files/best_model_weight_val_res50.pth'


def evaluation(model,direct):
    seg.train_model_outer(
        100,
        10,
        model,
        device=device,
    ...
    out = model(
        seg.transform_image(
            "./data/Dataset_Student/train/video_0/image_21.png"
        ).unsqueeze(0)
    )["out"][0]
